...
/* Inputs are {ap,rn} and {bp,rn}; output is {rp,rn}, computation is
   ...
   tp==rp is allowed. */
void
ntt_mpn_bc_mulmod_bnm1 (mp_ptr rp, mp_srcptr ap, mp_srcptr bp, mp_size_t rn,
		    mp_ptr tp)
{
  mp_limb_t cy;
  ...
  ntt_mpn_mul_opt (tp, ap, rn, bp, rn);
...
static void
ntt_mpn_bc_mulmod_bnp1 (mp_ptr rp, mp_srcptr ap, mp_srcptr bp, mp_size_t rn,
		    mp_ptr tp)
{
  mp_limb_t cy;
  ...
  ntt_mpn_mul_opt (tp, ap, rn + 1, bp, rn + 1);
  ASSERT (tp[2*rn+1] == 0);
 ...
 */
void
ntt_mpn_mulmod_bnm1 (mp_ptr rp, mp_size_t rn, mp_srcptr ap, mp_size_t an, mp_srcptr bp, mp_size_t bn, mp_ptr tp)
{
  ASSERT (0 < bn);
  ASSERT (bn <= an);
  ASSERT (an <= rn);
  ...
  if ((rn & 1) != 0 || BELOW_THRESHOLD (rn, MULMOD_BNM1_THRESHOLD))
      ...
      if (UNLIKELY (bn < rn))
	{
	  if (UNLIKELY (an + bn <= rn))
	    {
	      ntt_mpn_mul_opt (rp, ap, an, bp, bn);
	    }
	  else
	    {
	      mp_limb_t cy;
	      ntt_mpn_mul_opt (tp, ap, an, bp, bn);
	      cy = mpn_add (rp, tp, rn, tp + rn, an + bn - rn);
	      MPN_INCR_U (rp, rn, cy);
	    }
	}
      else
	ntt_mpn_bc_mulmod_bnm1 (rp, ap, bp, rn, tp);
    }
  else
    {
      mp_size_t n;
...
#define sp1 (tp + 2*n + 2)
	...
	ntt_mpn_mulmod_bnm1 (rp, n, am1, anm, bm1, bnm, so);
      }
	...
	/*if (k >= FFT_FIRST_K) {
	...
	else*/ if (UNLIKELY (bp1 == b0))
	  {
	    ...
	    ASSERT (anp + bnp > n);
	    ASSERT (anp >= bnp);
	    ntt_mpn_mul_opt (xp, ap1, anp, bp1, bnp);
	    anp = anp + bnp - n;
	  ...
	  ntt_mpn_bc_mulmod_bnp1 (xp, ap1, bp1, n, xp);
      }
