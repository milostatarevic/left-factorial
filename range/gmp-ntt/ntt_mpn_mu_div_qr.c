...
static mp_limb_t ntt_mpn_mu_div_qr2 (mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

static mp_limb_t
ntt_mpn_preinv_mu_div_qr (mp_ptr qp,
		      mp_ptr rp,
		      mp_srcptr np,
		      mp_size_t nn,
		      mp_srcptr dp,
  ...
  if (qn + MU_DIV_QR_SKEW_THRESHOLD < dn)
    {
	 ...
	 most significant limbs of each operand.  */
      qh = ntt_mpn_mu_div_qr2 (qp, rp + nn - (2 * qn + 1),
			   np + nn - (2 * qn + 1), 2 * qn + 1,
			   dp + dn - (qn + 1), qn + 1,
			   scratch);
      ...
      if (dn - (qn + 1) > qn)
	ntt_mpn_mul_opt (scratch, dp, dn - (qn + 1), qp, qn);  /* prod is dn-1 limbs */
      else
	ntt_mpn_mul_opt (scratch, qp, qn, dp, dn - (qn + 1));  /* prod is dn-1 limbs */
    ...
    {
      qh = ntt_mpn_mu_div_qr2 (qp, rp, np, nn, dp, dn, scratch);
    }
		...
		mp_ptr scratch)
{
  ...
  qh = ntt_mpn_preinv_mu_div_qr (qp, rp, np, nn, dp, dn, ip, in, scratch + in);
  ...
  while (qn > 0)
      ...
      if (qn < in)
	{
	 ...
	 by the upper part of the partial remainder R.  */
      ntt_mpn_mul_opt (tp, rp + dn - in, in, ip, in);		/* mulhi  */
	...
	ntt_mpn_mul_opt (tp, dp, dn, qp, in);		/* dn+in limbs, high 'in' cancels */
      else
	{
	  tn = mpn_mulmod_bnm1_next_size (dn + 1);
	  ntt_mpn_mulmod_bnm1 (tp, tn, dp, dn, qp, in, scratch_out);
