...
/*
  Divide the numerator (np,nn) by the denominator (dp,dn) and
...
void
ntt_mpn_tdiv_qr (mp_ptr qp, mp_ptr rp, mp_size_t qxn,
  ...
  switch (dn)
	...
	if (nn + adjust >= 2 * dn)
	  {
		...
		ntt_mpn_mu_div_qr (qp, rp, n2p, nn, d2p, dn, scratch);
		n2p = rp;
	      }
	...
	/* When we come here, the numerator/partial remainder is less
...

	  {
		...
		ntt_mpn_mul_opt (tp, qp, qn, dp, in);
	      }
	    else
	      ntt_mpn_mul_opt (tp, dp, in, qp, qn);
